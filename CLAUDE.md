...
Conductor is a spec-driven development framework that manages your project through a structured lifecycle: **Context -> Spec & Plan -> Implement**. It provides the following slash commands:

| Command | Description |
|---|---|
| `/conductor:setup` | Scaffolds the project and sets up the Conductor environment |
| `/conductor:newTrack` | Starts a new feature or bug track with spec and plan generation |
...
## Conductor Templates Path

When a command references `CONDUCTOR_PATH/templates/`, resolve it to the Conductor installation directory's `templates/` folder. The install script sets this path below:
