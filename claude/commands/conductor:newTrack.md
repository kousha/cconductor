...
2.  **Get Track Description:**
    ...
    *   **If `$ARGUMENTS` is empty:** Ask the user: "Please provide a brief description of the track (feature, bug fix, chore, etc.) you wish to start." Wait for their response before proceeding.
...
2.  **Questioning Phase:** Ask a series of questions to gather details for the `spec.md`. You must batch up to 4 related questions in a single message to streamline the process. Tailor questions based on the track type (Feature or Other).
    ...
    *   **General Guidelines:**
        ...
        *   **2. Formulate the Question:** For each question:
            - Include a short header label.
            - For exclusive choice questions, present 2-4 numbered options with descriptions.
            - For additive questions, present numbered options and tell the user they can select one or more.
            - For open-ended questions, ask for free-text input.

        *   **3. Interaction Flow:**
            *   Wait for the user's response after each set of questions.
            *   If the user wants something not covered by the options, ask a follow-up question to get their input.
...
4.  **User Confirmation:** Present the drafted specification to the user and ask:
    "Please review the drafted Specification below. Does this accurately capture the requirements?"
    1. **Approve** — The specification looks correct, proceed to planning.
    2. **Revise** — I want to make changes to the requirements.
    Wait for their response. Revise the `spec.md` content until confirmed.
...
3.  **User Confirmation:** Present the drafted Implementation Plan to the user and ask:
    "Please review the drafted Implementation Plan below. Does this look correct and cover all the necessary steps?"
    1. **Approve** — The plan looks solid, proceed to implementation.
    2. **Revise** — I want to modify the implementation steps.
    Wait for their response. Revise the `plan.md` content until confirmed.
