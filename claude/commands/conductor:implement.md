...
4.  **Select Track:**
    -   **If a track name was provided:**
        ...
        2.  If a unique match is found, ask the user: "I found track '<track_description>'. Is this correct?" (yes/no). Wait for their response.
        3.  If no match is found, or if the match is ambiguous, ask the user: "I couldn't find a unique track matching the name you provided. Did you mean '<next_available_track>'? Or please type the exact track name." Wait for their response.
    -   **If no track name was provided (or if the previous step failed):**
        ...
        2.  **If a next track is found:**
            -   Ask the user: "No track name provided. Would you like to proceed with the next incomplete track: '<track_description>'?" (yes/no). Wait for their response.
            -   If confirmed, proceed with this track. Otherwise, ask the user: "Please type the exact name of the track you would like to implement." Wait for their response.
...
4.  **Execute Tasks and Update Track Plan:**
    ...
    c. **For Each Task, You MUST:**
        i. **Defer to Workflow:** The **Workflow** file is the **single source of truth** for the entire task lifecycle. You MUST now read and execute the procedures defined in the "Task Workflow" section of the **Workflow** file you have in your context. Follow its steps for implementation, testing, and committing precisely.
           - **CRITICAL:** Every human-in-the-loop interaction, confirmation, or request for feedback mentioned in the **Workflow** (e.g., manual verification plans or guidance on persistent failures) MUST be conducted by asking the user directly and waiting for their response before proceeding.
...
5.  **Analyze and Update:**
    ...
    b.  **Update Product Definition:**
        ...
        ii. **Propose and Confirm Changes:** If an update is needed:
            -   Present the proposed updates (in diff format) to the user and ask: "Please review the proposed updates to the Product Definition below. Do you approve?" (yes/no). Wait for their response.
    ...
    c.  **Update Tech Stack:**
        ...
        ii. **Propose and Confirm Changes:** If an update is needed:
            -   Present the proposed updates (in diff format) to the user and ask: "Please review the proposed updates to the Tech Stack below. Do you approve?" (yes/no). Wait for their response.
    ...
    d. **Update Product Guidelines (Strictly Controlled):**
        ...
        iii. **Propose and Confirm Changes:** If the conditions are met:
            -   Present the proposed changes (in diff format) with a clear warning to the user and ask: "WARNING: This is a sensitive action as it impacts core product guidelines. Please review the proposed changes below. Do you approve these critical changes?" (yes/no). Wait for their response.
...
2.  **Ask for User Choice:** Ask the user: "Track '<track_description>' is now complete. What would you like to do?"
    1. **Review** — Run the review command to verify changes before finalizing.
    2. **Archive** — Move the track's folder to `conductor/archive/` and remove it from the tracks file.
    3. **Delete** — Permanently delete the track's folder and remove it from the tracks file.
    4. **Skip** — Do nothing and leave it in the tracks file.
...
3.  **Handle User Response:**
    ...
    *   **If user chooses "Delete":**
        i. **CRITICAL WARNING:** Ask the user: "WARNING: This will permanently delete the track folder and all its contents. This action cannot be undone. Are you sure?" (yes/no). Wait for their response.
