...
2.  **Auto-Detect Scope:**
    ...
    -   If one exists, ask the user: "Do you want to review the in-progress track '<track_name>'?" (yes/no). Wait for their response.
    -   If no track is in progress, or user says "no", ask the user: "What would you like to review? (Enter track name, or 'current' for uncommitted changes)". Wait for their response.
3.  **Confirm Scope:** Ask the user: "I will review: '<identified_scope>'. Is this correct?" (yes/no). Wait for their response.
...
3.  **Load and Analyze Changes (Smart Chunking):**
    ...
    -   **Strategy Selection:**
        ...
        -   **Large Changes (> 300 lines):**
            -   **Confirm:** Ask the user: "This review involves >300 lines of changes. I will use 'Iterative Review Mode' which may take longer. Proceed?" (yes/no). Wait for their response.
...
2.  **Action:**
    -   **If issues found:** Ask the user: "How would you like to proceed with the findings?"
...
2.  **Condition for Action:**
    ...
    -   If changes are detected:
        a. **Check for Track Context:**
            - If you are NOT reviewing a specific track (i.e., you don't have a `plan.md` in context), ask the user: "I've detected uncommitted changes. Should I commit them?" (yes/no). Wait for their response.
        ...
        b. **Handle Track-Specific Changes:**
            i.   **Confirm with User:** Ask the user: "I've detected uncommitted changes from the review process. Should I commit these and update the track's plan?" (yes/no). Wait for their response.
            ii.  **If Yes:**
...
2.  **Ask for User Choice:** Ask the user: "Review complete. What would you like to do with track '<track_name>'?"
...
3.  **Handle User Response:**
    ...
    *   **If "Delete":**
        i.   **Confirm:** Ask the user: "WARNING: This is an irreversible deletion. Do you want to proceed?" (yes/no). Wait for their response.
